...
#include "OneTask_WaitTargetDataUsingActor.h"

#include "AbilitySystemComponent.h"
#include "One/Common/AbilitySystem/TargetDataActor/OneGameplayAbilityTargetActor_LineTrace.h"


UOneTask_WaitTargetDataUsingActor* UOneTask_WaitTargetDataUsingActor::CreateTargetDataUsingActorProxy(UGameplayAbility* OwningAbility,
...
{
	UOneTask_WaitTargetDataUsingActor* MyObj = NewAbilityTask<UOneTask_WaitTargetDataUsingActor>(OwningAbility, TaskInstanceName);
...
void UOneTask_WaitTargetDataUsingActor::Activate()
{
...
void UOneTask_WaitTargetDataUsingActor::OnTargetDataReplicatedCallback(const FGameplayAbilityTargetDataHandle& Data, FGameplayTag ActivationTag)
{
...
void UOneTask_WaitTargetDataUsingActor::OnTargetDataReplicatedCancelledCallback()
{
	check(AbilitySystemComponent.Get());
	if (ShouldBroadcastAbilityTaskDelegates())
	{
...
void UOneTask_WaitTargetDataUsingActor::OnTargetDataReadyCallback(const FGameplayAbilityTargetDataHandle& Data)
{
	check(AbilitySystemComponent.Get());
	if (!Ability)
	{
...
void UOneTask_WaitTargetDataUsingActor::OnTargetDataCancelledCallback(const FGameplayAbilityTargetDataHandle& Data)
{
	check(AbilitySystemComponent.Get());
...
void UOneTask_WaitTargetDataUsingActor::ExternalConfirm(bool bEndTask)
{
	check(AbilitySystemComponent.Get());
	if (TargetActor)
	{
...
void UOneTask_WaitTargetDataUsingActor::ExternalCancel()
{
	check(AbilitySystemComponent.Get());
	if (ShouldBroadcastAbilityTaskDelegates())
	{
...
void UOneTask_WaitTargetDataUsingActor::InitializeTargetActor() const
{
	check(TargetActor);
	check(Ability);
	...
	TargetActor->TargetDataReadyDelegate.AddUObject(const_cast<UOneTask_WaitTargetDataUsingActor*>(this),
	                                                &UOneTask_WaitTargetDataUsingActor::OnTargetDataReadyCallback);
	TargetActor->CanceledDelegate.AddUObject(const_cast<UOneTask_WaitTargetDataUsingActor*>(this),
	                                         &UOneTask_WaitTargetDataUsingActor::OnTargetDataCancelledCallback);
}

void UOneTask_WaitTargetDataUsingActor::FinalizeTargetActor() const
{
	check(TargetActor);
	check(Ability);
...
void UOneTask_WaitTargetDataUsingActor::RegisterTargetDataCallbacks()
{
	...
	if (!bIsLocallyControlled)
	{
		...
		if (!bShouldProduceTargetDataOnServer)
		{
			...
			AbilitySystemComponent->AbilityTargetDataSetDelegate(SpecHandle, ActivationPredictionKey).AddUObject(
				this, &UOneTask_WaitTargetDataUsingActor::OnTargetDataReplicatedCallback);
			AbilitySystemComponent->AbilityTargetDataCancelledDelegate(SpecHandle, ActivationPredictionKey).AddUObject(
				this, &UOneTask_WaitTargetDataUsingActor::OnTargetDataReplicatedCancelledCallback);
...
void UOneTask_WaitTargetDataUsingActor::OnDestroy(bool AbilityEnded)
{
	if (TargetActor)
	{
		if (AOneGameplayAbilityTargetActor_LineTrace* TraceTargetActor = Cast<AOneGameplayAbilityTargetActor_LineTrace>(TargetActor))
		{
...
bool UOneTask_WaitTargetDataUsingActor::ShouldReplicateDataToServer() const
{
	if (!Ability || !TargetActor)
	{
		return false;
